...
def main():
    ...
    model_names = ["PredictiveMacdCrossoverModel", "PredictiveSma20CrossoverModel",]  # Add all your model names
        ...
        if st.button("Scan Stocks", key="btn3"):
            for so in options:
                ...
                try: 
                    ...
                    model = selected_model_class(so, train_until, data_source=st.session_state['data_source'])
                    model_results = model.run_train()
